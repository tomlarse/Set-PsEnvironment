...
Task InstallModules -depends Clean {
...
Task UnitTests -Depends ScriptAnalysis {
    ...
    $PesterResults = Invoke-Pester -path "$TestsPath"  -CodeCoverage "$ConfigPath\*.ps1" -OutputFile "$TestResultsPath\UnitTest.xml" -OutputFormat NUnitXml -PassThru
